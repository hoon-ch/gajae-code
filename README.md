...
notifications SDK and managed Telegram reference daemon. In a running GJC
session, open `/settings` → **Notifications** to configure or reconfigure
...
GJC keeps one safe long-poll owner per bot token so new sessions attach cleanly
instead of tripping Telegram 409 conflicts; a foreign owner is never taken over.
