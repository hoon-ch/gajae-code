...
Three lifecycle gates keep setup separate from runtime activation:
...
2. An eligible session starts its generic endpoint only when the session is
   effectively enabled. It supports complete Telegram, Discord, or Slack
   configuration, or the explicit environment path.
3. The Telegram daemon is ensured only after that endpoint is running and only
   for a complete global Telegram configuration. Discord-only, Slack-only, and
   environment-only sessions do not start a Telegram daemon.

For a GJC-spawned child, `notifications.sessionScope=primary` suppresses the
control surface to avoid duplicate topics; `all` permits it. `GJC_NOTIFICATIONS=1`
or `GJC_NOTIFICATIONS_TOKEN` explicitly opts that child in, but never overrides a
hard opt-out or a helper/subagent exclusion.

Session precedence is highest first:

1. `GJC_NOTIFY=off`, `0`, or `false` prevents the notification control surface
   for that process.
2. `GJC_NOTIFICATIONS=0` is a hard runtime opt-out.
3. Local `/notify off` disables only the current session.
4. `GJC_NOTIFICATIONS=1` or `GJC_NOTIFICATIONS_TOKEN` enables the explicit
   generic session path.
5. A complete global configuration enables notifications automatically.
6. Otherwise notifications stay off.
