...
The editor component does **not** exist in this work item. The showcase fixture
and capture script below deliberately emit deterministic placeholder captures
until Work item 7 wires this contract to the actual editor render. Those
placeholder artifacts prove the matrix/schema only; they are not visual
readiness evidence for the product screen.
...
`160x48` using `unicode-color`: **28 × 3 = 84** entries. Add exactly these
...
The expected manifest count is therefore **88 = (28 × 3) + 4**. Every key is
`{state_id}/{viewport}/{render_mode}`. Each entry directory contains
`terminal.txt`, `terminal-ansi.txt`, `terminal.html`, and `metadata.json`; the
root `manifest.json` lists all 88 entries and the SHA-256/byte length of every
entry file. Metadata records replay source, terminal size, fixed fixture
capture timestamp, rendering assumptions, wrapping policy, capture mode, and
whether it is placeholder or the future live editor render.

Work item 7 must replace the placeholder renderer with the real editor render,
then regenerate captures, inspect all relevant scroll positions, and obtain an
...
record both manifest counts as 88 plus CJK review results. Do not wire this
scaffold into CI until the live editor exists and the captures are meaningful.
