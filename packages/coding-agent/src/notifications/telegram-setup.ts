...
export type TelegramSetupFailureStatus = "error" | "cancelled" | "requires_explicit_chat" | "aborted";
 ...
 * boundary: only a verified same-token daemon with a safe chat path proceeds.
 */
export function resolveTelegramSetupPollingPolicy(input: {
	...
	if (!fingerprint) return { kind: "cancel_foreign_or_unknown_owner", identity: "unknown" };
...
export async function runTelegramSetup(input: RunTelegramSetupInput): Promise<TelegramSetupResult> {
	...
	const policy = resolveTelegramSetupPollingPolicy({ token, preflight: input.preflight, chatId: input.chatId });
	...
	const chat =
		policy.kind === "discover"
			? await discoverPrivateTelegramChat({
					token,
					deps: input.deps,
			...
			: await validateTelegramPrivateChat({ token, chatId: policy.chatId, deps: input.deps, signal: input.signal });
	...
	return {
		...
		pairingSource:
			policy.kind === "discover" ? "discovered" : policy.kind === "reuse_stored_chat" ? "reused" : "provided",
	};
}
