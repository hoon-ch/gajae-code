...
import { FileSessionStorage, type SessionStorageFileIdentity } from "../session-storage";
import {
	acquireManagedLock,
...
function legacyDirectoryNames(
	...
	// Legacy directories were named from lexical Win32 drive paths in that case.
	const legacyRelativeCwd = (root: string): string | undefined =>
		relativeTo(root, canonicalCwd) ?? (platform === "win32" ? relativeTo(root, lexicalCwd) : undefined);
	const homeRelative = legacyRelativeCwd(home);
	if (homeRelative !== undefined) {
		const encodedHome = encodeRelative(home);
		if (homeRelative === "") {
			names.add("----");
			names.add(`--${encodedHome}--`);
		} else {
			names.add(`---${encodeRelative(homeRelative)}--`);
			names.add(`--${encodedHome}-${encodeRelative(homeRelative)}--`);
		}
	}
	const tempRelative = legacyRelativeCwd(os.tmpdir());
	if (tempRelative !== undefined) names.add(`--${`-tmp-${encodeRelative(tempRelative)}`}--`);
	return [...names];
}
...
export function listManagedCandidates(scope: ManagedScope): ManagedCandidateListing {
	try {
		const root = fs.lstatSync(scope.sessionsRoot);
		if (!root.isDirectory() || root.isSymbolicLink())
		...
		const directories: Array<{ path: string; provenance: "v2" | "legacy" }> = [
			...
			...legacyDirectoryNames(scope.platform, scope.canonicalCwd, scope.legacyLexicalCwd).map(directoryName => ({
...
function receiptMatches(
	...
	try {
		...
		return (
			...
			validManifest &&
			manifestMatches(destination.path, manifest as readonly ArtifactManifestEntry[]) &&
			...
			record.destination?.sha256 === destination.identity.sha256 &&
			exact(record.destination?.identity, destination)
		);
	} catch {
		return false;
	}
...
function sameCleanupIdentity(left: SessionStorageFileIdentity, right: SessionStorageFileIdentity): boolean {
	return (
		left.dev === right.dev &&
		left.ino === right.ino &&
...
function probePlannedCleanupDetach(target: RetiredTarget, pending: CleanupReceipt): CleanupReceipt {
	...
	if (fs.existsSync(pending.plannedArtifactsPath)) {
		if (!pending.expectedArtifactsIdentity) throw new Error("durability_failed");
		const observed = artifactIdentityAt(pending.plannedArtifactsPath);
		if (!observed || !sameCleanupIdentity(observed, pending.expectedArtifactsIdentity))
			throw new Error("durability_failed");
		detachedArtifactsPath = pending.plannedArtifactsPath;
	}
		...
		for (const record of records) {
			...
			if (
				...
				((record.detachedArtifactsPath !== undefined &&
					record.detachedArtifactsPath !== latest.plannedArtifactsPath) ||
					(record.detachedTranscriptPath !== undefined &&
						record.detachedTranscriptPath !== latest.plannedTranscriptPath))
			)
				throw new Error("durability_failed");
...
function nextCleanupReceipt(target: RetiredTarget, pending: CleanupReceipt | undefined): CleanupReceipt {
	...
	return {
		attempt,
		target,
		expectedArtifactsIdentity: pending?.expectedArtifactsIdentity ?? artifactIdentityForCleanup(target),
...
function detachArtifactRootForMigration(plan: DetachedArtifactRoot): DetachedArtifactRoot {
	...
	if (!result.ok || !result.detachedPath || path.resolve(result.detachedPath) !== plan.detachedPath)
		throw new Error("durability_failed");
	return plan;
}
...
function restorePreparedArtifactRoot(scope: ManagedScope, source: ManagedCandidate): void {
	...
	let record: {
		sourceArtifactQuarantine?: { path?: unknown; detachedPath?: unknown; identity?: Record<string, unknown> };
	};
	try {
	...
	if (
		...
		throw new Error("durability_failed");
	if (fs.existsSync(quarantine.path)) return;
...
function migrationReceipt(
	...
	manifest: readonly ArtifactManifestEntry[],
	sourceArtifactQuarantine?: { path: string; detachedPath: string; identity: DetachedArtifactRoot["identity"] },
): Uint8Array {
	lock.assertOwned();
	...
	for (const name of fs.readdirSync(directory)) {
		...
		try {
			...
			for (const target of lockedTargets) {
				...
				const active = discoveredDetach
					? nextCleanupReceipt(target, observedPending)
					: (observedPending ?? nextCleanupReceipt(target, undefined));
				if (!observedPending || discoveredDetach) await publishCleanupPending(scope, tombstone, active, lock);
...
export async function openManagedCandidateForWrite(
	...
	try {
		...
		manifest = artifactManifest(afterLock.path, detachedArtifacts?.detachedPath);
	...
	try {
		...
		for (const target of targets) {
			...
			let deletion = await new FileSessionStorage().deleteSessionVerified({
				...
				transcriptIdentity: target.identity,
				expectedArtifactsIdentity: active.expectedArtifactsIdentity,
				detachedArtifactsPath: active.detachedArtifactsPath,
				detachedTranscriptPath: active.detachedTranscriptPath,
				plannedArtifactsPath: active.plannedArtifactsPath,
				plannedTranscriptPath: active.plannedTranscriptPath,
			});
			if (deletion.kind === "artifacts_removed") {
			...
			if (deletion.kind === "cleanup_pending") {
				if (
					(deletion.phase === "artifacts" && deletion.detachedArtifactsPath !== active.plannedArtifactsPath) ||
