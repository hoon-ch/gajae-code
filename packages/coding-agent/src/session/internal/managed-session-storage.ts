...
import { applyOwnerOnlyPathSecurity, verifyOwnerOnlyPathSecurity } from "@gajae-code/natives";
...
export async function publishManagedFileNoReplace(
	...
	try {
		...
		assertOwned?.();
		try {
			// The authority check and the no-replace publication must share one synchronous turn.
			fs.linkSync(staging, destination);
		} catch (error) {
			if ((error as NodeJS.ErrnoException).code === "EEXIST") throw new Error("destination_conflict");
			throw error;
		}
