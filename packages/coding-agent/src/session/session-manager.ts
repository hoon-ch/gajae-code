...
/** Internal artifact-writing capability. Public read-only facades never carry this authority. */
...
/**
 * Returns an artifact capability only for the concrete persistence owner. This deliberately
 * rejects structural lookalikes and every public ReadonlySessionManager facade.
 */
...
async function readTerminalBreadcrumb(cwd: string): Promise<string | null> {
	...
	try {
		...
		if ("kind" in inspected) return null;
		...
		return (
			listing.owned.reduce<ManagedCandidate | undefined>(
				(latest, candidate) =>
					!latest || candidate.identity.mtimeMs > latest.identity.mtimeMs ? candidate : latest,
				undefined,
			)?.path ?? null
		);
	} catch (err) {
...
async function movePathAcrossDevicesSafe(source: string, destination: string): Promise<void> {
	...
	if (!sourceStat.isDirectory()) {
		try {
			...
			try {
				await syncPathAndParent(path.dirname(source));
			} catch (error) {
				throw new PathMoveCompletedWithSyncFailure(error);
			}
			return;
		} catch (error) {
			if (error instanceof PathMoveCompletedWithSyncFailure) throw error;
...
export class SessionManager {
	...
	/** Tombstone and exact-delete managed transcripts; never delete the active transcript by pathname. */
	async dropSession(sessionPath: string): Promise<void> {
		...
		if (this.#sessionFile && path.resolve(this.#sessionFile) === requestedPath)
			throw new Error("Refusing to delete the active session transcript; switch sessions before deletion.");
		await this.#closePersistWriter();
		if (this.storage instanceof FileSessionStorage) {
			const sessionsRoot = path.dirname(this.sessionDir);
			const resolved = resolveManagedScope({
				cwd: this.cwd,
				agentDir: path.resolve(sessionsRoot, ".."),
				sessionsRoot,
			...
			if (resolved.kind === "error")
				throw new Error(`Managed session storage could not be resolved; refusing deletion: ${resolved.message}`);
		}
	...
	/** Open a default-managed candidate through the binding and copy-retention protocol. */
	static async #openDefaultManagedCandidateForWrite(
		filePath: string,
	...
	static async forkFrom(
		...
		const managedSourcePath =
			!sessionDir && storage instanceof FileSessionStorage
				? await SessionManager.#openDefaultManagedCandidateForWrite(sourcePath, migrationPolicy)
				: sourcePath;
		const dir = sessionDir ?? SessionManager.getDefaultSessionDir(cwd, undefined, storage);
		const manager = new SessionManager(cwd, dir, true, storage);
	...
	static async open(
		...
		const managedPath =
			!sessionDir && storage instanceof FileSessionStorage
				? await SessionManager.#openDefaultManagedCandidateForWrite(filePath, migrationPolicy)
				: filePath;
	...
	static async listForResumePickerReadOnly(
		...
		if (storage instanceof FileSessionStorage) {
			const listing = listManagedCandidates(resolved.scope);
			if (listing.kind === "error" || listing.invalid.length > 0) return [];
	...
	static async openExistingStrict(
		...
		if (!sessionDir && storage instanceof FileSessionStorage) {
			try {
				sessionPath = await SessionManager.#openDefaultManagedCandidateForWrite(sessionPath, migrationPolicy);
			} catch (error) {
