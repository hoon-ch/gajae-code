...
import type { NotificationSettingsSnapshot } from "@gajae-code/coding-agent/notifications/config";
import type {
	NotificationHealthReport,
	NotificationRecoveryReport,
} from "@gajae-code/coding-agent/notifications/notification-service";
import type {
	NotificationSessionReconcileResult,
	NotificationSessionStatus,
} from "@gajae-code/coding-agent/notifications/session-control";
...
describe("notification settings controller adapter", () => {
	it("maps editor operations to notification services, session control, and atomic commits", async () => {
		let currentSnapshot = snapshot({ enabled: false });
		...
		const dependencies: Partial<NotificationsEditorOperationDependencies> = {
			...
			recoverNotifications: async input => {
				...
				expect(input).toMatchObject({ settings, stateRoot: "/workspace/current" });
				return recovery();
			},
