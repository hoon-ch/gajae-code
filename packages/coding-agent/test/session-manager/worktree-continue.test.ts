...
describe("continueRecent with --worktree sessions", () => {
	...
	it("does not let a non-managed breadcrumb override logical managed inventory", async () => {
		...
		const managedFile = writeSession(worktree, "managed-worktree-session");
		const resumed = await SessionManager.continueRecent(repo);
		try {
			expect(resumed.getSessionFile()).toBe(managedFile);
		} finally {
			await resumed.close();
		}
	});
