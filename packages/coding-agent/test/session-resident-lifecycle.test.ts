...
describe("resident cache prune retention, lifecycle cleanup, and JSONL parity", () => {
	...
	it("keeps live resident text readable when moveTo session-file publication fails", async () => {
		...
		vi.spyOn(fs.promises, "link").mockImplementation(async (source, target) => {
			if (String(source) === sessionFile) throw new Error("simulated session publication failure");
			return realLink(source, target);
		});

		await expect(sm.moveTo(newRoot)).rejects.toThrow("simulated session publication failure");
	...
	it("keeps live resident text readable when artifact publication fails after session rollback", async () => {
		...
		vi.spyOn(fs.promises, "cp").mockImplementation(async (source, target, options) => {
			if (String(source) === oldArtifactDir) throw new Error("simulated artifact publication failure");
			return realCopy(source, target, options);
		});

		await expect(sm.moveTo(newRoot)).rejects.toThrow("simulated artifact publication failure");
		expect(sm.getCwd()).toBe(root);
		expect(sm.getSessionFile()).toBe(sessionFile);
		expect(fs.existsSync(sessionFile)).toBe(true);
		expect(JSON.stringify(sm.getEntries())).toContain(sentinel);
