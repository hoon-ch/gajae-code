...
describe("FileSessionStorage.deleteSessionWithArtifacts", () => {
	...
	it("retains the transcript when artifact cleanup fails", async () => {
		...
		await expect(storage.deleteSessionWithArtifacts(sessionPath)).rejects.toThrow(
			"Session cleanup is pending in artifacts: permission denied",
		);
		const detachedArtifactsPath = rmSpy.mock.calls[0]?.[0];
		expect(typeof detachedArtifactsPath).toBe("string");
		expect(path.dirname(String(detachedArtifactsPath))).toBe(tempDir);
		expect(path.basename(String(detachedArtifactsPath))).toMatch(/^\.gjc-delete-/);
		expect(fs.existsSync(sessionPath)).toBe(true);
		expect(fs.existsSync(artifactsDir)).toBe(false);
		expect(fs.existsSync(String(detachedArtifactsPath))).toBe(true);
	});
